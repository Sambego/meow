...
export default class AudioCodePage extends Component {
    ...
    codeExample6 = 'const ac = new AudioContext();\n\nconst oscillator = ac.createOscillator();\noscillator.frequency.value = 440;\noscillator.type = \'sine\';\noscillator.start();\n\noscillator.connect(ac.destination);';

    render() {
        return (
            <BubbleSlide previous="/audio-example" next="/vibration">
                <Bubble>Here are the basics of creating sounds with the web-audio api. We first need to create a new audio context.</Bubble>
                <Bubble full>
                    <Code code={this.codeExample1} />
                </Bubble>
                <Bubble>Once we have an audio context we can create an oscillator, which will generate a tone for us.</Bubble>
                <Bubble full>
                    <Code code={this.codeExample2} />
                </Bubble>
                <Bubble>We can adjust the frequency and type of our oscillator, and start it.</Bubble>
                <Bubble full>
                    <Code code={this.codeExample3} />
                </Bubble>
                <Bubble>The oscillator will not be audible until you connect it to the destination of the audio context.</Bubble>
                <Bubble full>
                    <Code code={this.codeExample4} />
                </Bubble>
                <Bubble>An audio context has a built in clock, you can access it to schedule sounds.</Bubble>
                <Bubble full>
