    ...
    render() {
        return (
            <BubbleSlide previous="/location-example" next="/notifications">
                ...
                {('Notification' in window) && <Notification message="Hey Sam, when are you coming home, I'm hungry!" />}
                {('Notification' in window) && <Notification message="Oh, by the way, these holes in the couch were already there when I got here 😇." icon={IconHoly}/>}
                {('Notification' in window) && <Notification message="Yeah, I found out how to send notifications in the browser." icon={IconRobot}/>}
            </BubbleSlide>
        );
    }
};
